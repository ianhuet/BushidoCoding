...
function hex2dec(hex) {
  let dec = hex.split('').reverse().map(function(n,i){
    return parseInt(n, 16) * (Math.pow(16,i));
  });
...
function geoHashing(dow, date) {
  ...
  dow = parseFloat(Math.round(dow * 100) / 100).toFixed(2);
  datum +=  "-" + dow;
  ...
  let geo = [hashed.substring(0, 15), hashed.substring(15)];
  console.log(geo);

  // convert to decimal
  geo = geo.map(function(n){
    let dec = hex2dec(n); // .toFixed(6);
    return dec;
  });
...
// MD5: db9318c2259923d08b672cb305440f97 => db9318c2259923d | 08b672cb305440f97

...
let input = `
  ...
  bssqn wbmdc rigc zketu ketichh enkixg bmdwc stnsdf jnz mqovwg inputxgken
...
// loop through array checking for duplicates
  // no duplicate found >> add to validPhrase array
...
Now, a valid passphrase must contain no two words that are anagrams of each other - that is, a passphrase 
is invalid if any word's letters can be rearranged to form any other word in the passphrase.
*/

function Day04_Part2(input){
  let checkSet;

  checkSet = input.trim().split('\n').map(
    n => n.trim().split(' ').map(
      m => m.split('').sort().join('')
  ));

  checkSet = checkSet.filter(n => n.every((e,i,n) => n.indexOf(e) === n.lastIndexOf(e)));
